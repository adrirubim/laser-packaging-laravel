...
- **Session:** `POST /locale` validates and stores `locale` in the session; `SetLocale` middleware applies it before each request.
...
- **Shared components** that already use `t()`: `ActionsDropdown`, `SearchInput` (default placeholder and clear button), `LocaleDropdown`, `LocaleTabs`, sidebar (`app-sidebar.tsx`). When adding new shared UI, use `useTranslations()` and add the keys to the three JSON files.
